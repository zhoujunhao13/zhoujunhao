...
public class Utils
{
	...
	public static String builderContentDispositionFileName(HttpHeaders headers,String filename)
	{
	    ...
	    try
	    {
            ...
            if (browser != null && browser.toLowerCase().indexOf("firefox") > 0) contentDisposition += "filename*=UTF-8''" + URLEncoder.encode(filename, "UTF-8");
            else contentDisposition += "filename="+URLEncoder.encode(filename, "UTF-8");
	    }
	    catch(Exception ex)
	    {
